...
st.set_page_config(page_title="Revisor IVE BDC25 Pro", layout="wide")
st.title("🛠️ Revisor de Instalaciones (IVE BDC25) - Mapeo Oficial")
st.write("Sube tu Excel y obtén la validación con códigos reales del árbol estructural del IVE.")

# --- BANCO DE PRECIOS E IDENTIFICADORES REALES IVE (BDC25) ---
# Corregido según el árbol estructural: EIEC (Tubos), EIEE (Enlace), EIEM (Mecanismos)
precios_ive = {import streamlit as st
import pandas as pd
import io
...
if uploaded_file:
    try:
        ...
        for index, fila in df.iterrows():
            ...
            try: precio_presu = float(fila.get(col_pres, 0))
            except: precio_presu = 0.0

            if not codigo or codigo == "None" or len(codigo) < 2: continue
...
# --- RADAR COMERCIAL (Mercado / Fabricantes) ---
cat_comercial = {
    "luminaria": {"marca": "Philips / Ledvance / Jiso", "precio": "35€ - 120€ / ud"},
    "proyector": {"marca": "Disano / Gewiss / Simon", "precio": "85€ - 380€ / ud"},
    "pantalla led": {"marca": "Philips CoreLine", "precio": "45€ - 95€ / ud"},
...
if uploaded_file:
    try:
        # Intentar cargar Hoja5, si falla, cargar primera hoja disponible
        try:
            df = pd.read_excel(uploaded_file, sheet_name='Hoja5')
        except:
            df = pd.read_excel(uploaded_file)
        
        # Limpieza robusta de nombres de columnas para evitar KeyErrors
        df.columns = [str(c).strip() for c in df.columns]
        
        # Identificación inteligente y dinámica de columnas basada en tus capturas
        col_codigo = next((c for c in df.columns if "cod" in c.lower() or c == "Presupuesto"), df.columns[0])
        ...
        for index, fila in df.iterrows():
            ...
            # Saltarse filas vacías, títulos de capítulos o nulos para limpiar la tabla
            ...
            # 1. EVALUACIÓN DE CONCORDANCIA DIRECTA IVE (CÓDIGO YA EXISTENTE)
            if codigo in precios_ive:
                p_ive_col = f"{precios_ive[codigo]['precio']} €"
                nuevo_codigo_ive = precios_ive[codigo]['codigo_oficial']
                if precio_presu <= precios_ive[codigo]['precio']:
            ...
            # --- 🔍 MOTOR DE OPTIMIZACIÓN CRUZADA CON CÓDIGOS CORREGIDOS (IVE ≥ PRESU) ---
            if p_ive_col == "—":
                for cod_ive_ref, info_ive in precios_ive.items():
                    if any(kw in texto_analisis for kw in info_ive["keywords"]):
                        # Regla de oro: El IVE oficial debe proteger o subir tu dinero
                        if info_ive["precio"] >= precio_presu:
                            nuevo_codigo_ive = info_ive['codigo_oficial']
                            p_ive_col = f"{info_ive['precio']} €"
                            val_texto = f"🔵 SUGERENCIA: CAMBIAR A CÓDIGO REAL IVE ({nuevo_codigo_ive})"
                            break
                        else:
                            val_texto += " | ⚠️ IVE Estructural detectado pero es más bajo (No usar para no perder dinero)"
        ...
        if resultados:
            df_final = pd.DataFrame(resultados)
            st.success("✅ Estructura indexada según el árbol real de familias BDC25 (EIEC, EIEE, EIEM).")
            st.dataframe(df_final, use_container_width=True)
            ...
            st.warning("No se pudieron procesar partidas válidas. Revisa el formato de las celdas.")
            
    except Exception as e:
        st.error(f"Error técnico en el análisis: {e}. Asegúrate de que el archivo contiene las columnas de precios correspondientes.")
